...
TASK
----
This is an iterative filtering step

Determine whether the discussion clearly maps to one specific component in this batch. Use SIG labels as an initial ownership/domain signal. If a SIG label clearly matches the email topic, first focus on components under that SIG. Then choose the best component_path using the description, area labels, source files, and code headers. Do not select a component based only on SIG label if several components share the same SIG.

- If the email discussion is mainly about process, coordination, meetings, releases, CI/testing workflow, general project management, or broad project-level concerns, return "NONE" unless there is clear evidence linking it to a specific component in this batch.
- If one component in this batch is clearly the best match based on the component metadata, select its component_path.
- If no component in this batch is sufficiently supported by the email discussion, return "NONE".

HOW TO INTERPRET EACH COMPONENT DESCRIPTION
-------------------------------------------
Each component contains the following information:
- Component Path: The directory path that identifies the component. This is the final label that may be selected.
...
HOW TO DECIDE
-------------
Use this process to select the best component from this batch:
1. Identify the main technical topic in the email thread.
2. Use SIG, area, and other labels to find potentially relevant components in this batch.
...
SELECTION RULES
---------------
- Return exactly one value: either one component_path from this batch, or "NONE".
- Do not invent or modify component paths.
- Do not select a component based only on broad SIG labels or keyword overlap.
- If multiple components in this batch are equally plausible, return "NONE".
- If the evidence is weak, return "NONE".
- Be conservative.
...
- The rationale must be 1-2 sentences and explain why this component is the best match, or why NONE was selected.
