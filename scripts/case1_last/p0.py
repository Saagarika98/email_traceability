...
OUTPUT_FILE = os.path.join(BASE_DIR, "results", "case1", "p0_last_result.json")
...
def build_prompt(email_thread, component_text):
    return f""" You are mapping a technical discussion to the most relevant system component.
...
TASK
----
First determine whether the discussion clearly maps to a specific component. Use SIG labels as an initial ownership/domain signal. 
If a SIG label clearly matches the email topic, first focus on components under that SIG. 
Then choose the final component_path using the description, area labels, source files, and code headers.
Do not select a component based only on SIG label if several components share the same SIG.

- If the email discussion is mainly about process, coordination, meetings, releases, CI/testing workflow, general project management, 
or broad project-level concerns, return "NONE" unless there is clear evidence linking it to a specific component.
...
- The rationale must be 1-2 sentences and explain why this component is the best match""".strip()
